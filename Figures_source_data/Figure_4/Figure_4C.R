...
#TFs_to_select = c("ZNF384", "ZNF773", "ZNF43", "GABPA")
TFs_to_select = c("CREB3L3", "GABPA",   "GLI4",    "ZFP3",    "ZNF107",  "ZNF226",  "ZNF234",  "ZNF322",  "ZNF43",   "ZNF772", "ZNF773",  "ZNF836")
...
image = xxyy %>%
  ...
  ylab("Average gain, GHT-SELEX to ChIP-Seq") +
  xlab("Average gain, ChIP-Seq to GHT-SELEX") + 
  geom_hline(yintercept=0, linetype="dotted") + 
  geom_vline(xintercept = 0, linetype="dotted")+
  
  geom_point(data = subset(xxyy, (xxyy$TF_name %in% TFs_to_select)),
             alpha=1, shape=21, color="white") +

  geom_text_repel(data = subset(xxyy, (xxyy$TF_name %in% TFs_to_select)|(xx < -0.01)|(xx > 0.17)|(yy< -0.07)|(yy > 0.1)),
                  aes(label = TF_name), 
                  point.padding = 1,
...
library(Cairo)
CairoFonts( usePUA=T)
Cairo(file='Figure_4C.pdf', type="pdf", width=310, height=185, units="mm")
